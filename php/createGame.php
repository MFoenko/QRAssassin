<?php
	...
	$result = mysqli_query($conn, "SELECT COUNT(game_id) FROM games WHERE game_name = $game_name);":
	...
	$result = mysqli_query($conn, "SELECT COUNT(game_id) FROM games WHERE admin_id = $admin_id AND winner = null;");
	$row = mysqli_fetch_row($result);
	if($row > 0){
	...
	mysqli_query($conn, "INSERT INTO games(admin_id,game_name, password, start_reg, end_reg, start_game) 
